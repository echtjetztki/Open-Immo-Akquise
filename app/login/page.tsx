...
export default function LoginPage() {
    const basicLicenseSeenKey = 'open-akquise-basic-license-prompt-seen';
    const basicLicenseActiveKey = 'open-akquise-basic-license-active';
    const demoSignupUrl =
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (ua.includes('OpenAkquiseAdmin')) {
    ...
    useEffect(() => {
        if (!mounted) return;
        const isActive = window.localStorage.getItem(basicLicenseActiveKey) === '1';
        setIsBasicLicenseActive(isActive);
        if (isActive) {
            return;
        }

        const hasSeenPrompt = window.localStorage.getItem(basicLicenseSeenKey) === '1';
        if (!hasSeenPrompt) {
            setShowBasicLicenseModal(true);
            window.localStorage.setItem(basicLicenseSeenKey, '1');
        }
    }, [basicLicenseActiveKey, basicLicenseSeenKey, mounted]);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (response.status === 403) {
                    ...
                    window.localStorage.removeItem(basicLicenseActiveKey);
                }
            }
        } catch {
    ...
    const verifyBasicLicense = async () => {
        ...
        try {
            ...
            setBasicLicenseKey('');
            window.localStorage.setItem(basicLicenseActiveKey, '1');
            window.localStorage.removeItem(basicLicenseSeenKey);
        } catch {
