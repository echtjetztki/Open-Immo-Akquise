...
import { hashPassword, isLegacyPlaintextPasswordHash, verifyPassword } from '@/lib/password';
import { checkActiveLicenseForRequest } from '@/lib/license';
import { encodeSessionValue } from '@/lib/session';
...
async function getTeamUserByPassword(password: string): Promise<DbUserRow | null> {
    ...
    for (const row of result.rows) {
        const candidate = row as DbUserRow;
        const storedHash = candidate.password_hash ?? '';
        if (!verifyPassword(password, storedHash)) {
            continue;
        }

        if (isLegacyPlaintextPasswordHash(storedHash)) {
            const upgradedHash = hashPassword(password);
            await query(
                'UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
                [upgradedHash, candidate.id]
            );
            candidate.password_hash = upgradedHash;
        }
...
async function verifyPasswordAndUpgradeIfNeeded(user: DbUserRow | null, password: string): Promise<boolean> {
    if (!user) {
        return false;
    }
...
async function getDemoUsersFromDb(): Promise<{ adminUser: DbUserRow | null; userUser: DbUserRow | null }> {
    await ensureUsersTable();
    const result = await query(
        `SELECT id, username, role, display_name, password_hash
         FROM users
         WHERE (username = 'admin' AND role = 'admin')
            OR (username = 'user' AND role = 'user')`
    );

    let adminUser: DbUserRow | null = null;
    let userUser: DbUserRow | null = null;

    for (const row of result.rows) {
        const candidate = row as DbUserRow;
        if (candidate.username === 'admin' && candidate.role === 'admin') {
            adminUser = candidate;
        } else if (candidate.username === 'user' && candidate.role === 'user') {
            userUser = candidate;
        }
    }

    return { adminUser, userUser };
}
...
export async function POST(request: Request) {
    try {
        ...
        const demoCredentialsOnly = isDemoReadOnly() || normalizeFlag(process.env.DEMO_CREDENTIALS_ONLY);
        const demoAdminPw = (process.env.DEMO_ADMIN_PASSWORD || '').trim();
        const demoUserPw = (process.env.DEMO_USER_PASSWORD || '').trim();
        const demoUsersFromDb = await getDemoUsersFromDb();
        const demoAdminMatches = demoAdminPw
            ? password === demoAdminPw
            : await verifyPasswordAndUpgradeIfNeeded(demoUsersFromDb.adminUser, password);
        const demoUserMatches = demoUserPw
            ? password === demoUserPw
            : await verifyPasswordAndUpgradeIfNeeded(demoUsersFromDb.userUser, password);
        const isDemoCredentialAttempt = demoAdminMatches || demoUserMatches;
        const cookieStore = await cookies();
        const allowDemoBypass = normalizeFlag(process.env.LICENSE_ALLOW_DEMO_BYPASS);
        const bypassLicenseWithDemoCredentials = allowDemoBypass && isDemoCredentialAttempt;

        let basicLicenseVerified = false;
        if (!bypassLicenseWithDemoCredentials) {
            try {
                ...
                const message = error instanceof Error ? error.message : 'unknown_license_error';
                await sendSecurityWebhook(request, {
                    event: 'login_failed',
                    reason: 'license_backend_unavailable',
                    mode,
                ...
                return NextResponse.json(
                    { success: false, error: 'Lizenzpruefung derzeit nicht erreichbar. Bitte spaeter erneut versuchen.' },
                    { status: 503 }
                );
            }
        } else {
            basicLicenseVerified = true;
        }

        if (!basicLicenseVerified && !bypassLicenseWithDemoCredentials) {
            await sendSecurityWebhook(request, {
                event: 'login_failed',
                reason: 'basic_license_required',
                mode,
                targetAgent: agentInput,
                source: 'api/login'
            });
            return NextResponse.json(
                { success: false, error: 'Bitte zuerst Basis-Lizenz-Code aktivieren.' },
                { status: 403 }
            );
        }
        ...
        if (demoCredentialsOnly || isDemoCredentialAttempt) {
            if (isAgentLogin) {
            ...
            const adminPw = (process.env.DEFAULT_ADMIN_PASSWORD || process.env.ADMIN_PASSWORD || '').trim();
            ...
            if (isAgentLogin) {
                ...
                try {
                    ...
                    if (dbAgent) {
                        const storedHash = dbAgent.password_hash ?? '';
                        if (!verifyPassword(password, storedHash)) {
                            await sendSecurityWebhook(request, {
                                event: 'login_failed',
                                reason: 'wrong_password',
                        ...
                        if (isLegacyPlaintextPasswordHash(storedHash)) {
                            ...
                            await query(
                                'UPDATE users SET password_hash = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
                                [upgradedHash, dbAgent.id]
                            );
                            dbAgent.password_hash = upgradedHash;
                        }
        ...
        const signedSession = await encodeSessionValue(authUser);
        ...
        const message =
            rawMessage === 'session_secret_not_configured'
                ? 'SESSION_SECRET fehlt. Bitte serverseitig konfigurieren.'
                : rawMessage;
